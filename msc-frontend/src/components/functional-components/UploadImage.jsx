...
class UploadImage extends React.Component {
  ...
  handleForm(currentForm, prevState){
    if(prevState.selectedImage != this.state.selectedImage && this.state.selectedImage != null) {
      ...
      axios({
        method: "put",
        url: "http://10.61.44.90:8080/api/v1/forms/" + currentForm.formId,
        data: currentForm
      }).then(() => {
        this.setState({ isImageUploaded: true });
  ...
  handleUploadFile(file) {
    ...
    if (validExtensions.includes(file.type)) {
      ...
      axios({
        url: "http://10.61.44.90:8080/api/v1/upload",
        method: "POST",
        data: formData,
        headers: {
          ...
          this.setState({selectedImage : "http://10.61.44.90:8080/api/v1/files/" + res.data.id});
        });
      // animation
